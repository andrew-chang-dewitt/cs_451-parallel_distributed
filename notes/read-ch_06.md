---
title: "Parallel & Distributed: low-level programming (reading)"
...
```no-linenums
set args    run         break / clear
continue    next        step / stepi
disassembly info registers
...
compiles (with gcc, x86-64, no optimization) to:

```asm
summation_int:
  endbr64
  leal (%rdi,%rsi), %eax   ; add ints, result in eax (32-bit)
  ret

summation_int_to_long:
  endbr64
  add  %esi, %edi
  movslq %edi, %rax         ; sign-extend 32→64 bit result
  ret

summation_long:
  endbr64
  leaq (%rdi,%rsi), %rax   ; add longs, result in rax (64-bit)
  ret
```
...
support; requires the `LOCK` prefix in parallel (multi-core) settings.
